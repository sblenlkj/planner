...
Проект начинался как идея личного ассистента, который помогает пользователю управлять задачами, сроками и продуктивностью. Сейчас основной пользовательский вход в систему идет через Telegram-бота: пользователь пишет `/start`, Telegram Gateway создаёт пользователя в Backend и привязывает Telegram IDs, а дальше Agent Service интерпретирует намерение, вызывает типизированные tools, а Backend сохраняет изменения в доменной модели.
...
- автоматическое создание пользователя в Backend при первом `/start`;
...
Telegram Gateway не владеет бизнес-логикой. Он знает только transport-level информацию: Telegram ID, Telegram Chat ID, данные профиля Telegram и текущую Redis-сессию.
...
Обычное сообщение пользователя после `/start` проходит такой путь:

```text
1. Пользователь отправляет `/start` в Telegram.
2. Telegram Gateway получает webhook update.
3. Gateway проверяет, есть ли binding для Telegram пользователя.
4. Если binding нет, Gateway создаёт пользователя в Backend, сохраняет binding и отвечает пользователю.
5. Если binding уже есть, Gateway достаёт текущую Redis-сессию.
6. Gateway добавляет сообщение пользователя в сессию.
7. Gateway отправляет историю сообщений в Agent Service.
8. Agent Service проверяет security/input guard.
9. Agent Service загружает planner context пользователя из Backend.
10. Agent Service получает LLM slot.
11. Agent запускается с набором tools.
12. Если нужно, agent вызывает один или несколько tools.
13. Tools обращаются к Backend через HTTP adapters.
14. Backend исполняет доменные use cases.
15. Agent Service возвращает assistant_text.
16. Gateway сохраняет user/assistant messages в Redis-сессию.
17. Gateway отправляет ответ пользователю.
```
...
5. Открыть Telegram-бота и отправить `/start`.
6. Проверить, что Gateway создал пользователя в Backend и привязал Telegram chat.
7. Отправить обычное сообщение боту и посмотреть ответ агента.
8. Проверить Backend state.
9. Проверить Langfuse trace.
10. При необходимости закрыть сессию через `/agent/session/close`.
11. Проверить, что session-close workflow создал `day observation`.
...
- Telegram onboarding через `/start` и автоматическую привязку пользователя;
...
- доработать Telegram onboarding, timezone handling и UX первого контакта;
