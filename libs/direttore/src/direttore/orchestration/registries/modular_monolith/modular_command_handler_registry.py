from __future__ import annotations

from collections.abc import Iterable

from direttore.orchestration.base_classes.uow import (
    AbstractCommandUnitOfWork,
)
from direttore.orchestration.base_types.message import Command
from direttore.orchestration.registries.service.command_handler_registry import (
    CommandHandlerRegistry,
)


class ModularMonolithCommandHandlerRegistry(CommandHandlerRegistry):
    """
    Internal modular-monolith command registry.

    User code should normally register command handlers in a regular
    CommandHandlerRegistry.

    Modular runtime converts regular registries into this registry by attaching
    root command UoW metadata for every command registration.
    """

    def __init__(self, *, source_name: str | None = None) -> None:
        super().__init__(source_name=source_name)
        self._root_uow_type_by_command_type: dict[
            type[Command],
            type[AbstractCommandUnitOfWork],
        ] = {}

    @classmethod
    def from_registry(
        cls,
        *,
        registry: CommandHandlerRegistry,
        root_uow_type: type[AbstractCommandUnitOfWork],
        source_name: str | None = None,
    ) -> ModularMonolithCommandHandlerRegistry:
        cls._validate_static_uow_type(root_uow_type)

        modular_registry = cls(
            source_name=source_name or registry.source_name,
        )

        for registration in registry.iter_registrations():
            modular_registry._add_registration(registration)
            modular_registry._add_root_uow_type(
                command_type=registration.command_type,
                root_uow_type=root_uow_type,
            )

        return modular_registry

    @classmethod
    def merge_many(
        cls,
        registries: Iterable[ModularMonolithCommandHandlerRegistry],
        *,
        source_name: str | None = None,
    ) -> ModularMonolithCommandHandlerRegistry:
        merged = cls(source_name=source_name)

        for registry in registries:
            for registration in registry.iter_registrations():
                merged._add_registration(registration)

            for command_type, root_uow_type in (
                registry._root_uow_type_by_command_type.items()
            ):
                merged._add_root_uow_type(
                    command_type=command_type,
                    root_uow_type=root_uow_type,
                )

        return merged

    @classmethod
    def from_registries(
        cls,
        contexts: Iterable[
            tuple[CommandHandlerRegistry, type[AbstractCommandUnitOfWork]]
        ],
        *,
        source_name: str | None = None,
    ) -> ModularMonolithCommandHandlerRegistry:
        return cls.merge_many(
            (
                cls.from_registry(
                    registry=registry,
                    root_uow_type=root_uow_type,
                )
                for registry, root_uow_type in contexts
            ),
            source_name=source_name,
        )

    def get_root_uow_type_by_command_type(
        self,
        command_type: type[Command],
    ) -> type[AbstractCommandUnitOfWork]:
        self._validate_command_type(command_type)

        root_uow_type = self._root_uow_type_by_command_type.get(command_type)

        if root_uow_type is None:
            raise LookupError(
                "No root command unit-of-work type registered for command. "
                f"Command={command_type.__module__}.{command_type.__qualname__}."
            )

        return root_uow_type

    def get_root_uow_type(
        self,
        command: Command,
    ) -> type[AbstractCommandUnitOfWork]:
        return self.get_root_uow_type_by_command_type(type(command))

    def _add_root_uow_type(
        self,
        *,
        command_type: type[Command],
        root_uow_type: type[AbstractCommandUnitOfWork],
    ) -> None:
        self._validate_command_type(command_type)
        self._validate_uow_type(root_uow_type)

        existing_root_uow_type = self._root_uow_type_by_command_type.get(
            command_type
        )

        if (
            existing_root_uow_type is not None
            and existing_root_uow_type is not root_uow_type
        ):
            raise ValueError(
                "Duplicate root command unit-of-work type registration. "
                f"Command={command_type.__module__}.{command_type.__qualname__}, "
                f"existing_uow={existing_root_uow_type.__module__}."
                f"{existing_root_uow_type.__qualname__}, "
                f"new_uow={root_uow_type.__module__}.{root_uow_type.__qualname__}."
            )

        self._root_uow_type_by_command_type[command_type] = root_uow_type

    def _validate_uow_type(
        self,
        uow_type: type[AbstractCommandUnitOfWork],
    ) -> None:
        self._validate_static_uow_type(uow_type)

    @staticmethod
    def _validate_static_uow_type(
        uow_type: type[AbstractCommandUnitOfWork],
    ) -> None:
        if not isinstance(uow_type, type):
            raise TypeError(f"UoW type must be a type, got {uow_type!r}.")

        if not issubclass(uow_type, AbstractCommandUnitOfWork):
            raise TypeError(
                f"{uow_type.__module__}.{uow_type.__qualname__} "
                "must inherit from AbstractCommandUnitOfWork."
            )